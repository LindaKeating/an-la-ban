...
class Options {
	...
	/**
	 * Holds list of default countries for Apple Podcast review fetching.
	 *
	 * @since 7.3.0
	 * @access private
	 * @var array
	 */
	private $default_countries = array();
	...
	public function __construct() {
		...
		$this->default_countries = array(
			'US' => esc_html__( 'United States', 'podcast-player' ),
			'KR' => esc_html__( 'South Korea', 'podcast-player' ),
			'ES' => esc_html__( 'Spain', 'podcast-player' ),
			'AU' => esc_html__( 'Australia', 'podcast-player' ),
			'SE' => esc_html__( 'Sweden', 'podcast-player' ),
			'GB' => esc_html__( 'United Kingdom', 'podcast-player' ),
			'NO' => esc_html__( 'Norway', 'podcast-player' ),
			'NL' => esc_html__( 'Netherlands', 'podcast-player' ),
			'AR' => esc_html__( 'Argentina', 'podcast-player' ),
			'DE' => esc_html__( 'Germany', 'podcast-player' ),
			'FR' => esc_html__( 'France', 'podcast-player' ),
			'IT' => esc_html__( 'Italy', 'podcast-player' ),
			'IN' => esc_html__( 'India', 'podcast-player' ),
			'CA' => esc_html__( 'Canada', 'podcast-player' ),
			'MX' => esc_html__( 'Mexico', 'podcast-player' ),
			'BR' => esc_html__( 'Brazil', 'podcast-player' ),
		);
	}
	...
	public static function init() {
		...
		add_action( 'wp_ajax_pp_fetch_appleid', array( $inst, 'fetch_apple_id' ) );
		add_action( 'wp_ajax_nopriv_pp_fetch_appleid', array( $inst, 'fetch_apple_id' ) );
		add_action( 'wp_ajax_pp_fetch_reviews', array( $inst, 'fetch_podcast_reviews' ) );
		add_action( 'wp_ajax_nopriv_pp_fetch_reviews', array( $inst, 'fetch_podcast_reviews' ) );
		add_action( 'wp_ajax_pp_delete_reviews', array( $inst, 'delete_podcast_reviews' ) );
		add_action( 'wp_ajax_nopriv_pp_delete_reviews', array( $inst, 'delete_podcast_reviews' ) );
	}
	...
	/**
	 * Try to fetch Apple podcast ID from the feed.
	 *
	 * @since 1.0.0
	 */
	public function fetch_apple_id() {
		check_ajax_referer( 'podcast-player-admin-options-ajax-nonce', 'security' );
		...
		if ( ! $fprn ) {
			$output = array(
				'status'  => 'error',
				'message' => esc_html__( 'Invalid feed key provided.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		$store_manager = StoreManager::get_instance();
		$reviews       = $store_manager->get_misc_data( $fprn, 'pp_apple_reviews' );

		if ( $reviews ) {
			$output = array(
				'status'  => 'available',
				'id'      => $reviews['appleID'],
				'ccode'   => $reviews['countries'],
				'reviews' => $reviews['reviews'],
				'message' => sprintf( 'There are %d stored reviews for this podcast from %d countries.', count( $reviews['reviews'] ), count( $reviews['countries'] ) ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}

		$feed = Get_Fn::get_feed_data( $fprn, array( 'start' => 0, 'end' => 1 ), array( 'title' ) );
		if ( is_wp_error( $feed ) ) {
			$error  = esc_html( $feed->get_error_message() );
			$output = array(
				'status'  => 'error',
				'message' => $error,
			);
		...
		$title  = isset( $feed['title'] ) ? $feed['title'] : '';
		$author = isset( $feed['author'] ) ? $feed['author'] : '';
		$furl   = isset( $feed['furl'] ) ? $feed['furl'] : '';

		if ( ! $title || ! $furl ) {
			$output = array(
				'status'  => 'error',
				'message' => esc_html__( 'Unable to find Podcast Data.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		$term = $title . ' ' . $author;
		$apple_search_api_url = 'https://itunes.apple.com/search?media=podcast&entity=podcast&term=' . urlencode( $term );

		$content = wp_safe_remote_request(
			$apple_search_api_url,
			array(
				'timeout' => 30,
				'sslverify' => false,
			)
		...
		if ( is_wp_error( $content ) ) {
			$error = esc_html( $content->get_error_message() );
			$output = array(
				'status'  => 'error',
				'message' => $error,
			);
		...
		$apple_id = false;
		$results  = isset( $content['results'] ) ? $content['results'] : array();

		foreach ( $results as $result ) {
			$feed_url = isset( $result['feedUrl'] ) ? $result['feedUrl'] : '';
			if ( untrailingslashit($furl) === untrailingslashit($feed_url) ) {
				$apple_id = isset( $result['trackId'] ) ? $result['trackId'] : '';
				break;
			}
		}
		...
		if ( ! $apple_id ) {
			$output = array(
				'status'  => 'error',
				'message' => esc_html__( 'Unable to find Apple Podcast ID. Try entering manually in the below field.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
	...
	/**
	 * Try to fetch Apple podcast reviews.
	 *
	 * @since 1.0.0
	 */
	public function fetch_podcast_reviews() {
		check_ajax_referer( 'podcast-player-admin-options-ajax-nonce', 'security' );

		$apple_id = isset( $_POST['appleid'] ) ? absint( wp_unslash( $_POST['appleid'] ) ) : '';
		$ccode    = isset( $_POST['ccode'] ) ? sanitize_text_field( wp_unslash( $_POST['ccode'] ) ) : '';
		$podcast  = isset( $_POST['podcast'] ) ? sanitize_text_field( wp_unslash( $_POST['podcast'] ) ) : '';
		if ( ! $apple_id || ! $ccode || ! $podcast ) {
			$output = array(
				'error' => esc_html__( 'Invalid Information provided.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		$store_manager = StoreManager::get_instance();
		$db_reviews    = $store_manager->get_misc_data( $podcast, 'pp_apple_reviews' );
		$db_reviews    = $db_reviews ? $db_reviews : array();

		$countries = array( $ccode );
		$reviews = array();
		$countries_with_reviews = array();
		foreach ( $countries as $country ) {
			$country = strtolower( $country );
			$page_num = 1;
			for ( $page_num = 1; $page_num < 11; $page_num ++ ) {
				$review_url = "https://itunes.apple.com/$country/rss/customerreviews/page=$page_num/id=$apple_id/sortby=mostrecent/json";
				$content = Get_Fn::get_external_link_data( $review_url );
				if ( is_wp_error( $content ) ) {
					break;
				}

				$entries = isset( $content['feed']['entry'] ) ? $content['feed']['entry'] : array();
				
				// If entries are less than 50, there should not be any more pages.
				if ( count( $entries ) < 50 ) {
					$page_num = 11;
				}
				...
				foreach ( $entries as $entry ) {
					$review = array();
					$id = isset( $entry['id']['label'] ) ? absint( $entry['id']['label'] ) : false;
					if ( ! $id ) {
						continue;
					}
					$review['rating']  = isset( $entry['im:rating']['label'] ) ? absint( $entry['im:rating']['label'] ) : '';
					$review['title']   = isset( $entry['title']['label'] ) ? sanitize_text_field( $entry['title']['label'] ) : '';
					$review['content'] = isset( $entry['content']['label'] ) ? sanitize_text_field( $entry['content']['label'] ) : '';
					$review['date']    = isset( $entry['updated']['label'] ) ? sanitize_text_field( $entry['updated']['label'] ) : '';
					$review['author']  = isset( $entry['author']['name']['label'] ) ? sanitize_text_field( $entry['author']['name']['label'] ) : '';
					$reviews[ strval( $id ) ] = $review;
					$countries_with_reviews[] = $country;
				}
			}
		}
		$countries_with_reviews = array_values( array_filter( array_unique( $countries_with_reviews ) ) );

		if ( empty( $reviews ) ) {
			$output = array(
				'data' => array(
					'id'      => $db_reviews['appleID'],
					'ccode'   => $db_reviews['countries'],
					'message' => sprintf( 'We have collected %d reviews for this podcast from %d countries.', count( $db_reviews['reviews'] ), count( $db_reviews['countries'] ) ),
				),
			);
			echo wp_json_encode( $output );
			wp_die();
		...
		$prev_reviews            = isset( $db_reviews['reviews'] ) ? $db_reviews['reviews'] : array();
		$prev_countries          = isset( $db_reviews['countries'] ) ? $db_reviews['countries'] : array();
		$db_reviews['appleID']   = $apple_id;
		$db_reviews['reviews']   = $prev_reviews + $reviews;
		$db_reviews['countries'] = array_values( array_filter( array_unique( array_merge( $prev_countries, $countries_with_reviews ) ) ) );

		$store_manager->update_misc_data( $podcast, 'pp_apple_reviews', $db_reviews );
	...
	/**
	 * Try to delete Apple podcast reviews.
	 *
	 * @since 1.0.0
	 */
	public function delete_podcast_reviews() {
		check_ajax_referer( 'podcast-player-admin-options-ajax-nonce', 'security' );

		$podcast  = isset( $_POST['podcast'] ) ? sanitize_text_field( wp_unslash( $_POST['podcast'] ) ) : '';
		if ( ! $podcast ) {
			$output = array(
				'error' => esc_html__( 'Invalid Podcast provided.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		$store_manager = StoreManager::get_instance();
		$status = $store_manager->delete_misc_data( $podcast, 'pp_apple_reviews' );
		if ( $status ) {
			$output = array(
				'message' => esc_html__( 'Podcast reviews deleted successfully.', 'podcast-player' ),
			);
			echo wp_json_encode( $output );
			wp_die();
		}
		...
		$output = array(
			'error' => esc_html__( 'Could not delete podcast reviews.', 'podcast-player' ),
		);
		echo wp_json_encode( $output );
		wp_die();
	}
