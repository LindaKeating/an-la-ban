...
class Prepare_Storage_New {
	...
	/**
	 * Apply changes to items level data.
	 *
	 * @since 6.5.0
	 *
	 * @param array $new_data Newly Created data for the item.
	 * @param array $old Old data for the item.
	 */
	private function apply_items_changes( $new_data, $old ) {
		$deleted_items = array_diff_key( $old, $new_data );
		$del_ids       = array();
		$is_changed    = false;
		...
		if ( 'yes' !== $keep_old ) {
			$old = array_intersect_key( $old, $new_data );
		}
	 ...
	 */
	public function item_changes( $new_data, $old ) {
		$is_changed = false;
		$new_item   = $new_data->getVars();
